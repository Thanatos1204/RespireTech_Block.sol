...
const Signup = () => {
    ...
    const [dosageFrequency, setDosageFrequency] = useState(); // Default dosage frequency
    const [timeSlots, setTimeSlots] = useState(['']);


    const router = useRouter();  
    const { user, googleSignIn , register } = UserAuth();
    ...
    const handleSubmit = async (e) => {
      ...
      const sendEmailReminders = () => {
        // Logic to send email alerts and reminders using Email.js
        // Iterate through each time slot and send email reminders
        timeSlots.forEach(timeSlot => {
            // Example Email.js usage (replace with your actual Email.js configuration)
            emailjs.send('service_8wijzmi', 'template_lg3y1r2', {
                to_email: email,
            }, '6uhxtVkBP80sJLD42')
            .then((response) => {
    ...
    const handleDosageFrequencyChange = (e) => {
        const frequency = parseInt(e.target.value);
        setDosageFrequency(frequency);
        // Reset time slots if dosage frequency changes
        setTimeSlots(Array.from({ length: frequency }, () => ''));
    };

    return (
        <main className="w-full flex">
            <div className="relative flex-1 hidden items-center justify-center h-screen bg-[url('/levon.png')] lg:flex">
                <div className="relative z-10 w-full max-w-md">
                    <img src="/logo1.png" width={300} height={150} />
                    <div className=" mt-16 space-y-3">
                        <h3 className="text-white text-3xl font-bold">Start Monitoring Your Health Quickly</h3>
            ...
            <div className="flex-1 flex items-center justify-center bg-white h-screen">
                <div className="w-full max-w-md space-y-8 px-4 bg-white text-gray-600 sm:px-0">
                    ...
                    <div className="grid grid-cols-3 gap-x-3">
                        <button onClick={async ()=>{ await googleSignIn}} className="flex items-center justify-center py-2.5 border rounded-lg hover:bg-gray-50 duration-150 active:bg-gray-100">
                    ...
                    >
                        ...
                        <div>
                    <label className="font-medium">Dosage Frequency</label>
                    <input
                        type="number"
                ...
                <div>
                    <label className="font-medium">Preferred Time Slots</label>
